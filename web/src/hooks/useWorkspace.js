...
export default function useWorkspace() {
  ...
  const createWorkspace = () => {
    ...
    if (!name || !key) return;
    runAction(async () => {
      const data = await jsonRequest("/workspaces", "POST", { workspace: { name, key } });
  ...
  const createEpic = () => {
    ...
    runAction(async () => {
      const data = await jsonRequest(`/workspaces/${selected.id}/epics`, "POST", { epic: { name } });
  ...
  const createIssue = () => {
    ...
    runAction(async () => {
      const data = await jsonRequest(`/workspaces/${selected.id}/issues`, "POST", {
        issue: { title, issue_type: "task", status: "to_do", sprint_id: sprint?.id },
      });
  ...
  const createSprint = () => {
    ...
    if (!name || !selected) return;
    runAction(async () => {
      const data = await jsonRequest(`/workspaces/${selected.id}/sprints`, "POST", { sprint: { name, status: "planned" } });
      setSprints((current) => [...current, data.sprint]);
    });
  };

  const updateSprint = (sprint, attributes) => runAction(async () => {
    const data = await jsonRequest(`/sprints/${sprint.id}`, "PATCH", { sprint: attributes });
    setSprints((current) => current.map((item) => item.id === sprint.id ? data.sprint : item));
  });

  const deleteSprint = (sprint) => {
    if (!window.confirm(`Delete "${sprint.name}"? Issues will become unassigned.`)) return;
    runAction(async () => {
  ...
  return {
    workspaces, selected, epics, issues, sprints, error, loading,
    workspaceLoading, busy, selectedIssue, setSelectedIssue, setIssues,
    setError, loadWorkspaces, selectWorkspace, createWorkspace, createEpic,
    createIssue, createSprint, updateSprint, deleteSprint, reset,
  };
}
